...
func TestFeedFanoutConsumer_Process_ValidPayload(t *testing.T) {
	...
	body := buildEnvelope(t, publisher.PostCreatedPayload{PostID: "post-abc", UserID: "author-1"})
...
func TestFeedFanoutConsumer_Process_NoFollowers(t *testing.T) {
	...
	body := buildEnvelope(t, publisher.PostCreatedPayload{PostID: "post-xyz", UserID: "author-2"})
	if err := consumer.Process(body); err != nil {
		t.Fatalf("Process: %v", err)
	}
...
func TestFeedFanoutConsumer_Process_FollowStoreError(t *testing.T) {
	...
	body := buildEnvelope(t, publisher.PostCreatedPayload{PostID: "post-1", UserID: "author-1"})
	if err := consumer.Process(body); err == nil {
